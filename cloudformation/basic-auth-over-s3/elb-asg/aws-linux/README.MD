...
## Description
This Cloudformation template allows you to set up Basic Authentication with forced HTTPS over an S3 Bucket. By default S3 does not support Basic Authentication or HTTPS.
You can disable Basic Authentication by setting BasicAuthModify to off. For example if you have stg and production. 
...
- **BasicAuthModify**  
Set to 'off' to disable or change the name of the realm.
...
- **AllowIPFrom**  
IP address from which SSH (and possibly http(s)) will be allowed - the command to SSH will be like: *ssh -i {key-pair.pem} ec2-user@{ec2 instance ip}*  
- **AllowHTTPFrom**  
IP address from which HTTP/HTTPS will be allowed from. If not set it will use the AllowIPFrom param.
- **VpcId**  
